...
def _reward_terms(env):
    return {
        name: float(values[0]) for name, values in env.reward_manager.get_active_iterable_terms(0)
    }
...
def _set_root_pose(env, *, z: float, roll: float = 0.0) -> None:
    ...
    pose[0, 3:7] = quat_from_euler_xyz(torch.tensor([roll], device=env.device), zero, zero)[0]
    env_id = torch.tensor([0], dtype=torch.long, device=env.device)
    robot.write_root_link_pose_to_sim(pose, env_ids=env_id)
    robot.write_root_link_velocity_to_sim(torch.zeros((1, 6), device=env.device), env_ids=env_id)
    env.sim.forward()
    env.sim.sense()


def _place_lowest_wheel_on_support(env, *, roll: float, penetration_m: float = 0.002) -> None:
