        ...
        <aside className="runs-side-column">
          <section className="runs-panel">
            ...
            <form className="runs-form" onSubmit={createRun}>
              ...
              {['Ascento-Balance-Flat', 'Ascento-Velocity-Flat'].includes(createForm.task) && <label>Initial episode horizon (seconds)<select value={createForm.episode_horizon_s} onChange={(event) => setCreateForm({ ...createForm, episode_horizon_s: event.target.value })}><option value="20">20 seconds — frequent reset practice</option><option value="60">60 seconds</option><option value="120">120 seconds</option><option value="300">300 seconds — protected final phase</option></select><small>Promotes through 20 → 60 → 120 → 300 seconds after six consecutive 512-episode windows at 90% timeout completion. Shorter stages require sustained severe regression before demotion; the final 300-second phase stays active and saves long-horizon candidates for deterministic gate evaluation.</small></label>}
