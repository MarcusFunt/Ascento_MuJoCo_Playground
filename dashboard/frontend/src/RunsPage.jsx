        ...
        <aside className="runs-side-column">
          <section className="runs-panel">
            ...
            <form className="runs-form" onSubmit={createRun}>
              ...
              {['Ascento-Balance-Flat', 'Ascento-Velocity-Flat'].includes(createForm.task) && <label>Initial episode horizon (seconds)<select value={createForm.episode_horizon_s} onChange={(event) => setCreateForm({ ...createForm, episode_horizon_s: event.target.value })}><option value="20">20 seconds — frequent reset practice</option><option value="60">60 seconds</option><option value="120">120 seconds</option><option value="300">300 seconds — longest horizon</option></select><small>Promotes through 20 → 60 → 120 → 300 seconds after three 512-episode windows at 90% timeout completion. Two windows below 70% demote one stage so the policy can restabilize.</small></label>}
