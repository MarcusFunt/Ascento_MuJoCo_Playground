...
const ADVANCED_CHARTS = [
  { key: 'leg_target_offset_rms_rad', label: 'Leg target offset RMS (rad)', description: 'RMS displacement of the four policy-requested leg-position targets from the nominal pose.', interpretation: 'This is a structured position-target diagnostic. High demand can be useful during recovery or jumping, but sustained extremes may indicate pose hunting.' },
  { key: 'wheel_target_velocity_rms_rad_s', label: 'Wheel target velocity RMS (rad/s)', description: 'RMS requested wheel-speed target after the policy-to-target mapping.', interpretation: 'It is not wheel motion or applied torque. Compare it with drift/evaluation evidence to detect unnecessary wheel-command demand.' },
  { key: 'controller_request_saturation_fraction', label: 'Controller-request saturation', description: 'Fraction of controller torque requests clamped at the configured ±65 Nm request limit.', interpretation: 'High values mean the PD/PI controller is demanding all available authority; it is distinct from measured actuator output.' },
  { key: 'effort_rms', label: 'Controller-request RMS (Nm)', description: 'RMS requested controller effort when the trainer emits it.', interpretation: 'This is requested torque, not a direct policy-effort action and not measured actuator force.' },
  { key: 'effort_mean_abs', label: 'Mean controller request (Nm)', description: 'Mean absolute requested controller effort when the trainer emits it.', interpretation: 'Useful for distinguishing sustained controller demand from brief request spikes.' },
  { key: 'physical_saturation_fraction', label: 'Actuator-output saturation', description: 'Fraction of measured actuator output at a configured physical limit when emitted.', interpretation: 'This differs from controller-request saturation: motor response and torque-speed limits can prevent request from becoming output.' },
