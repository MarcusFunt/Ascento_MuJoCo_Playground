...
def _exact_reset(base_env: ManagerBasedRlEnv, scenarios: list[ScenarioSpec]) -> torch.Tensor:
    ...
    robot.write_root_link_pose_to_sim(torch.cat([positions, orientation], dim=-1), env_ids=env_ids)
...
def _refresh_exact_observation_history(base_env: ManagerBasedRlEnv) -> None:
    ...
    base_env.obs_buf = base_env.observation_manager.compute(update_history=True, env_ids=env_ids)
    ...
    *,
    ...
    for env_id, scenario in enumerate(scenarios):
        for disturbance in scenario.disturbances:
            if not (
                disturbance.start_step <= step < disturbance.start_step + disturbance.duration_steps
            ):
                continue
...
def _termination_reasons(base_env: ManagerBasedRlEnv, env_ids: torch.Tensor) -> dict[int, str]:
    ...
    for env_id in env_ids.detach().cpu().tolist():
        ...
        non_timeout = [name for name in names if not bool(manager.get_term_cfg(name).time_out)]
    ...
    try:
        for step in range(max_horizon):
            ...
            if is_recovery_task:
                ...
                recovery_stable_count = torch.where(
                    ...
                    torch.where(
                        active, torch.zeros_like(recovery_stable_count), recovery_stable_count
                    ),
                )
            ...
            newly_recovered = (
                after_disturbance & ~recovered & (stable_count >= stable_steps_required)
            )
        ...
        for index, scenario in enumerate(scenarios):
            ...
            if scenario.disturbances:
                ...
                metrics["disturbance_equivalent_delta_v"] = float(disturbance.equivalent_delta_v)
            results.append(
                EpisodeResult(
