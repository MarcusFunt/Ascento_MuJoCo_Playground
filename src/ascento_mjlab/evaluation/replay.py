...
class _ReplayPolicy:
  ...
  def __call__(self, _observations):
    ...
    self.base_env.scene["robot"].write_external_wrench_to_sim(
      forces, torques, body_ids=[0]
    )
...
def replay(
  *,
  ...
  print(f"[INFO] Horizon: {scenario.horizon_steps} steps ({scenario.horizon_steps * base_env.step_dt:.3f}s)")
  print("[INFO] Manual viewer reset is intentionally not scenario-aware; restart this command for an exact replay.")
