...
class HorizonCurriculumRunner(MjlabOnPolicyRunner):
    """Adapt the episode horizon to sustained success and regression.

    Each completion window contains a fixed number of finished vector slots.
    The horizon advances after three consecutive windows with at least 90%
    timeouts.  Two windows below 70% at a promoted stage demote one level,
    restoring shorter reset practice before the policy is asked to sustain the
    ...
    timeout_failure_threshold = 0.70
    ...
    def _evaluate_completion_window(self) -> None:
        ...
        self._emit_status(timeout_fraction=timeout_fraction, transition=transition)
        self._completed_in_window = 0
        self._timeouts_in_window = 0
    ...
    def _emit_status(self, *, timeout_fraction: float | None, transition: str = "held") -> None:
        parts = [
