...
def nonfinite(env: ManagerBasedRlEnv, asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG) -> torch.Tensor:
  asset: Entity = env.scene[asset_cfg.name]
  values = (asset.data.joint_pos, asset.data.joint_vel, asset.data.root_link_pos_w, asset.data.root_link_quat_w)
  return torch.stack([~torch.isfinite(value).all(dim=tuple(range(1, value.ndim))) for value in values], dim=0).any(dim=0)
