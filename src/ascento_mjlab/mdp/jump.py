...
from .events import flat_ground_wheel_bottom_heights
...
def initialize_jump_state(env, env_ids: torch.Tensor | None = None) -> None:
    ...
    # request as a fresh jump attempt on the next policy transition.
    motion_term = _motion_term(env)
    if motion_term is not None and hasattr(motion_term, "jump_generation"):
        state["last_jump_generation"][ids] = motion_term.jump_generation[ids]
    else:
...
def _forward_xy_from_quat(quat_wxyz: torch.Tensor) -> torch.Tensor:
    ...
    forward = torch.stack([1.0 - 2.0 * (y.square() + z.square()), 2.0 * (x * y + w * z)], dim=1)
    return forward / torch.linalg.vector_norm(forward, dim=1, keepdim=True).clamp(min=1.0e-8)


def update_jump_state(env, env_ids: torch.Tensor | None = None, dt: float | None = None) -> None:
