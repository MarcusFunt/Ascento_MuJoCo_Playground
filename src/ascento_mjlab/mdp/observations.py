...
def wheel_contacts(
  ...
  for name in (left_sensor_name, right_sensor_name):
    ...
    values.append((sensor.data.found > 0).float().flatten(start_dim=1).amax(dim=1, keepdim=True))
...
def wheel_contact_forces(
  ...
  for name in (left_sensor_name, right_sensor_name):
    ...
    values.append(torch.linalg.vector_norm(sensor.data.force, dim=-1).flatten(start_dim=1).amax(dim=1, keepdim=True))
  return torch.cat(values, dim=1)
